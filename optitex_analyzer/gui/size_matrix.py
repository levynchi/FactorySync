...
class SizeMatrixFrame(ttk.Frame):
    ...
    def set_sizes(self, sizes):
        ...
        for idx, size in enumerate(sizes):
            ...
            tk.Label(self, text=size, font=('Arial', 9, 'bold')).grid(row=row, column=col, padx=3, pady=(2, 0))
            var = tk.StringVar()
    ...
    def _build_empty_state(self):
        if self._allow_free_entry:
            tk.Label(self, text='מידה:', font=('Arial', 9)).grid(row=0, column=3, padx=3)
            free_size = tk.Entry(self, textvariable=self._free_size_var, width=10, justify='center')
            free_size.grid(row=0, column=2, padx=3)
            tk.Label(self, text='כמות:', font=('Arial', 9)).grid(row=0, column=1, padx=3)
            ...
            tk.Label(self, text=self._hint, fg='#7f8c8d', font=('Arial', 9)).grid(row=0, column=0, padx=4, pady=4)
            self._entries = []
