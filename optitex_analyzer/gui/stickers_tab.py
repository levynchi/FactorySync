...
BABY_BASIC_CATEGORY = 'בייבי בייסיק חמישיות'
...
class StickersTabMixin:
    def _create_stickers_tab(self):
        ...
        self._create_label_generator_tabs()
    ...
    def _on_stickers_tab_change(self):
        ...
        try:
            ...
            elif current_tab == 2:  # Label generator (אריה)
                if hasattr(self, '_label_gen_frame'):
                    self._label_gen_frame.refresh_products()
            elif current_tab == 3:  # Label generator (בייבי בייסיק)
                if hasattr(self, '_label_gen_bb_frame'):
                    self._label_gen_bb_frame.refresh_products()
        except Exception:
            pass
    ...
    # ==================== LABEL GENERATOR TABS ====================
    def _create_label_generator_tabs(self):
        """סאב-טאבים ליצירת מדבקות: אריה (ברירת מחדל) ובייבי בייסיק."""
        gen_tab = ttk.Frame(self.stickers_sub_notebook)
        self.stickers_sub_notebook.add(gen_tab, text="יצירת מדבקות")
        self._label_gen_frame = LabelGeneratorFrame(
            gen_tab,
            self.data_processor,
            logo_path=None,
            default_category='הכל',
            title='יצירת דף מדבקות מתבנית',
            sumatra_print_fn=self._print_pdf_sumatra,
            default_pack_size=3,
        )
        self._label_gen_frame.pack(fill="both", expand=True)

        bb_tab = ttk.Frame(self.stickers_sub_notebook)
        self.stickers_sub_notebook.add(bb_tab, text="יצירת מדבקות בייבי בייסיק")
        self._label_gen_bb_frame = LabelGeneratorFrame(
            bb_tab,
            self.data_processor,
            logo_path=LOGO_BABY_BASIC_PATH,
            default_category=BABY_BASIC_CATEGORY,
            title='יצירת דף מדבקות בייבי בייסיק',
            sumatra_print_fn=self._print_pdf_sumatra,
            draw_border=False,
            default_pack_size=5,
        )
        self._label_gen_bb_frame.pack(fill="both", expand=True)
